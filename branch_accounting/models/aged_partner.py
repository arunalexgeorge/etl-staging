...
class AccountMove(models.Model):
    ...
    branch_name = fields.Char('Branch Name', related='branch_id.name', store=True, index=True)
...
class AccountAgedPartner(models.AbstractModel):
    ...
    def _common_custom_unfold_all_batch_data_generator_receivable(self, internal_type, report, options, lines_to_expand_by_function):
        rslt = {} # In the form {full_sub_groupby_key: all_column_group_expression_totals for this groupby computation}
        report_periods = 17 # The report has 6 periods
            ...
            for line_to_expand in lines_to_expand: # In standard, this loop will execute only once
                if expand_function_name == '_report_expand_unfoldable_line_with_groupby':
                    ...
                    for column_group_key, column_group_options in report._split_options_per_column_group(options).items():
                        ...
                        aml_data_by_partner = {}
                        for aml_id, aml_result in self._aged_partner_report_custom_engine_receivable(column_group_options, internal_type, 'id', None):
    ...
    def _common_custom_unfold_all_batch_data_generator_payable(self, internal_type, report, options, lines_to_expand_by_function):
        rslt = {} # In the form {full_sub_groupby_key: all_column_group_expression_totals for this groupby computation}
        report_periods = 9 # The report has 6 periods
            ...
            for line_to_expand in lines_to_expand: # In standard, this loop will execute only once
                if expand_function_name == '_report_expand_unfoldable_line_with_groupby':
                    ...
                    for column_group_key, column_group_options in report._split_options_per_column_group(options).items():
                        # Get all aml results by partner
                        aml_data_by_partner = {}
                        for aml_id, aml_result in self._aged_partner_report_custom_engine_payable(column_group_options, internal_type, 'id', None):
                            aml_result['aml_id'] = aml_id
                            aml_data_by_partner.setdefault(aml_result['partner_id'], []).append(aml_result)
    ...
    def _report_custom_engine_aged_receivable(self, expressions, options, date_scope, current_groupby, next_groupby, offset=0, limit=None):
        return self._aged_partner_report_custom_engine_receivable(options, 'asset_receivable', current_groupby, next_groupby, offset=offset, limit=limit)
    ...
    def _report_custom_engine_aged_payable(self, expressions, options, date_scope, current_groupby, next_groupby, offset=0, limit=None):
        return self._aged_partner_report_custom_engine_payable(options, 'liability_payable', current_groupby, next_groupby, offset=offset, limit=limit)
    
    def _aged_partner_report_custom_engine_receivable(self, options, internal_type, current_groupby, next_groupby, offset=0, limit=None):
        ...
        periods = [
            ...
            (minus_days(date_to, 361), minus_days(date_to, 450)),
            (minus_days(date_to, 451), minus_days(date_to, 540)),
            (minus_days(date_to, 541), minus_days(date_to, 630)),
            (minus_days(date_to, 631), minus_days(date_to, 720)),
            (minus_days(date_to, 721), minus_days(date_to, 810)),
            (minus_days(date_to, 811), minus_days(date_to, 900)),
            (minus_days(date_to, 900), minus_days(date_to, 990)),
            (minus_days(date_to, 991), minus_days(date_to, 1080)),
            (minus_days(date_to, 1081), False),
        ]
        ...
        def build_result_dict(report, query_res_lines):
            ...
            if current_groupby == 'id':
                ...
                rslt.update({
                    ...
                    'branch_name': query_res['branch_name'][0] if query_res['branch_name'] else None,
                    'partner_name': query_res['partner_name'][0] if len(query_res['partner_name']) == 1 else None,
        ...
        query = f"""
            ...
            SELECT
                ...
                ARRAY_AGG(DISTINCT account_move_line.date) AS report_date,
                ...
                ARRAY_AGG(DISTINCT account_move_line.date) AS due_date,
    ...
    def _aged_partner_report_custom_engine_payable(self, options, internal_type, current_groupby, next_groupby, offset=0, limit=None):
        report = self.env['account.report'].browse(options['report_id'])
        report._check_groupby_fields((next_groupby.split(',') if next_groupby else []) + ([current_groupby] if current_groupby else []))
...
class AgedPayableCustomHandler(models.AbstractModel):
    ...
    def _custom_unfold_all_batch_data_generator(self, report, options, lines_to_expand_by_function):
        ...
        if self.env.ref('account_reports.aged_payable_line').groupby.replace(' ', '') == 'partner_id,id':
            return self._common_custom_unfold_all_batch_data_generator_payable('liability_payable', report, options, lines_to_expand_by_function)
        return {}
...
class AgedReceivableCustomHandler(models.AbstractModel):
    ...
    def _custom_unfold_all_batch_data_generator(self, report, options, lines_to_expand_by_function):
        ...
        if self.env.ref('account_reports.aged_receivable_line').groupby.replace(' ', '') == 'partner_id,id':
            return self._common_custom_unfold_all_batch_data_generator_receivable('asset_receivable', report, options, lines_to_expand_by_function)
        return {}