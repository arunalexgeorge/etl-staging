...
class AccountReportController(http.Controller):
    ...
    def get_report(self, options, file_generator, **kwargs):
        ...
        branch_str = request.httprequest.cookies.get('bids', str(request.env.user.branch_id.id))
        allowed_branch_ids = [int(str_id) for str_id in branch_str.split(',')]
        report = request.env['account.report'].with_user(uid).with_context(allowed_company_ids=allowed_company_ids, allowed_branch_ids=allowed_branch_ids).browse(options['report_id'])
